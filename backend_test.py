...
def test_ml_models():
    ...
    try:
        ...
        print(
            f"  ✅ URL Analysis: ML enabled={url_result['ml_enabled']}, Threat={url_result['threat_probability']:.3f}"
        )

        # Test content analysis
        content_result = engine.analyze_content_ml("This is a test content")
        print(
            f"  ✅ Content Analysis: ML enabled={content_result['ml_enabled']}, Phishing={content_result['phishing_probability']:.3f}"
        )
    ...
    try:
        ...
        for threat_score, malicious_count, suspicious_count, expected_level, expected_malicious in test_cases:
            ...
            if level == expected_level and malicious == expected_malicious:
                ...
                print(
                    f"  ❌ Test case failed: Score={threat_score}, Counts=({malicious_count},{suspicious_count}) -> Expected: {expected_level}, Got: {level}"
                )
                all_passed = False
