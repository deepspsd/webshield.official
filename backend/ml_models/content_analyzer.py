...
class ContentPhishingDetector:
    ...
    def _load_threat_patterns(self):
        ...
        return {
            'phishing_keywords': [
                ...
                'billing', 'payment', 'urgent', 'expire', 'limited', 'act-now',
                'validate', 'restore', 'unlock', 'frozen', 'locked', 'alert',
                'suspended', 'restricted', 'immediately', 'required', 'mandatory'
            ],
            'urgency_patterns': [
                'act now', 'urgent', 'immediate', 'expires today', 'expires soon',
                'limited time', 'act fast', 'don\'t miss', 'last chance',
                'time sensitive', 'deadline', 'hurry', 'quick action'
            ],
            'credential_requests': [
                'enter password', 'confirm password', 'verify identity',
                'social security', 'credit card', 'bank account', 'pin code',
                'security code', 'cvv', 'mother maiden name', 'date of birth'
            ],
            'threat_words': [
                'hacked', 'compromised', 'breach', 'unauthorized', 'suspicious',
                'illegal', 'violation', 'terminated', 'deactivated', 'blocked'
            ],
            'reward_scams': [
                'winner', 'won', 'prize', 'lottery', 'jackpot', 'million',
                'congratulations', 'selected', 'chosen', 'lucky', 'free gift'
            ],
            'form_actions': [
                'submit', 'confirm', 'verify', 'validate', 'proceed',
                'continue', 'next', 'login', 'sign in', 'authenticate'
            ]
        }
    ...
    def _load_legitimate_patterns(self):
        ...
        return {
            'security_indicators': [
                'https', 'ssl', 'encrypted', 'secure connection', 'privacy policy',
            ...
            'professional_terms': [
                'customer service', 'support team', 'help center', 'contact us',
                'privacy', 'legal', 'about us', 'careers', 'press'
            ]
        }
    ...
    def extract_content_features(self, html_content: str) -> Dict[str, float]:
        ...
        features['url_shortener_count'] = sum(1 for href in link_hrefs if any(short in href for short in url_shorteners))
        ...
        features['suspicious_link_count'] = sum(1 for href in link_hrefs if any(pattern in href for pattern in suspicious_link_patterns))
        ...
        features['security_badge_count'] = sum(1 for alt in image_alts if any(badge in alt for badge in security_badges))
        
        # Security indicators vs actual security
        features['security_claim_count'] = sum(1 for indicator in self.legitimate_patterns['security_indicators'] if indicator in text_content)
        features['has_https_links'] = 1 if any('https://' in href for href in link_hrefs) else 0
        features['has_http_links'] = 1 if any('http://' in href and 'https://' not in href for href in link_hrefs) else 0
    ...
    def predict(self, content: str, content_type: str = 'html') -> Dict[str, float]:
        ...
        if features.get('credential_request_score', 0) > 8:
            detected_indicators.append(f"Multiple credential requests detected ({features['credential_request_score']} found)")
