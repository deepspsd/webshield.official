...
try:
    from .auth_checker import check_email_authentication, AuthCheckResult
...
def analyze_sender_reputation(sender_email: str, sender_name: Optional[str] = None) -> SenderReputation:
    ...
    trusted_domains = {
        ...
        "amazon.com", "aws.amazon.com", "facebook.com", "meta.com", "instagram.com",
        ...
        # Enterprise/SaaS
        "salesforce.com", "adobe.com", "dropbox.com", "slack.com", "zoom.us",
        "atlassian.com", "jira.com", "notion.so", "figma.com", "canva.com",
        # E-commerce
        "ebay.com", "shopify.com", "etsy.com", "walmart.com", "target.com",
        # Streaming
        "netflix.com", "spotify.com", "hulu.com", "disney.com",
        # Other trusted
        "gov", "edu", "mil"  # Government, education, military TLDs
    }
    ...
    is_disposable = domain in disposable_providers or any(d in domain for d in ["tempmail", "throwaway", "disposable", "temp-mail", "fakeinbox"])
    ...
    if sender_name:
        ...
        severe_patterns = [
            r"verify.*account", r"urgent.*action", r"suspended.*account",
            r"confirm.*identity", r"update.*payment", r"security.*alert",
            r"password.*expire", r"account.*locked", r"unusual.*activity",
            r"limited.*time", r"act.*now", r"immediate.*action"
        ]
        # Moderate red flags
        moderate_patterns = [
            r"prize", r"winner", r"congratulations", r"claim.*reward",
            r"free.*offer", r"special.*promotion"
        ]
    ...
    suspicious_local_patterns = [
        r"^no-?reply$", r"^noreply$", r"support[0-9]+", r"service[0-9]+",
        r"[0-9]{6,}", r"^admin[0-9]+", r"^info[0-9]+"
    ]
...
def analyze_headers(headers: Optional[EmailHeaders], sender_email: Optional[str] = None) -> HeaderAnalysis:
    ...
    if AUTH_CHECKER_AVAILABLE and sender_email:
        try:
            ...
            logger.info(f"DNS Auth check results - SPF: {spf_status}, DKIM: {dkim_status}, DMARC: {dmarc_status}, Score: {auth_result.overall_score}")
...
def analyze_links(links: List[str]) -> LinkAnalysis:
    ...
    url_shorteners = [
        ...
        r"buff\.ly", r"ow\.ly", r"adf\.ly", r"j\.mp", r"v\.gd",
        r"cutt\.ly", r"rb\.gy", r"shorturl\.at", r"tiny\.cc"
    ]
    ...
    suspicious_patterns = [
        ...
        r"suspended.*account", r"unusual.*activity", r"secure.*login",
        r"password.*reset", r"account.*locked", r"urgent.*action",
        r"\.xyz\/", r"\.tk\/", r"\.ml\/", r"\.cf\/", r"\.gq\/",  # Free TLDs often used in phishing
        r"login.*\?", r"signin.*\?", r"auth.*\?",  # Login with query params
    ...
    malicious_patterns = [
        r"phishing", r"malware", r"virus", r"trojan", r"ransomware",
        r"free.*money", r"click.*here.*win", r"congratulations.*won",
        r"nigerian.*prince", r"lottery.*winner", r"inheritance.*claim",
        r"wire.*transfer", r"western.*union", r"bitcoin.*payment",
        r"\.exe$", r"\.scr$", r"\.bat$", r"\.cmd$", r"\.ps1$",  # Executable extensions
        r"download.*invoice", r"download.*receipt",
        r"paypal.*verify", r"amazon.*verify", r"apple.*verify", r"microsoft.*verify",
        r"bank.*of.*america.*verify", r"wells.*fargo.*verify",
        r"password.*expire", r"account.*terminate",
    ]
...
def calculate_threat_score(
    ...
    has_explicit_auth_failure = (
        header_analysis.spf_status == "fail" or
        header_analysis.dkim_status == "fail" or
        header_analysis.dmarc_status == "fail"
    )
    
    is_auth_unknown = (
        header_analysis.spf_status == "unknown" and 
        header_analysis.dkim_status == "unknown"
    )
...
async def scan_email_metadata(request: EmailScanRequest):
    ...
    try:
        ...
        sender_rep = analyze_sender_reputation(
            request.email_metadata.sender_email,
            request.email_metadata.sender_name
        )
        
        # Analyze headers with real DNS auth checking
        header_analysis = analyze_headers(
            request.email_metadata.headers,
...
async def check_email_auth(email: str):
    ...
    if not AUTH_CHECKER_AVAILABLE:
        raise HTTPException(
            status_code=503,
            detail="Email authentication checker not available"
        )
