...
class LLMService:
    ...
    async def generate_explanation(self, 
                                   ...
                                   vt_analysis: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            if groq_text:
                ...
                if isinstance(parsed, dict):
                    return {
                        ...
                        'risk_summary': str(parsed.get('risk_summary', '')).strip() or self._generate_risk_summary(context),
                        'threat_factors': parsed.get('threat_factors', context['threat_factors']) if isinstance(parsed.get('threat_factors', None), list) else context['threat_factors'],
                        'safety_factors': parsed.get('safety_factors', context['safety_factors']) if isinstance(parsed.get('safety_factors', None), list) else context['safety_factors'],
                        'recommended_action': str(parsed.get('recommended_action', '')).strip() or context['recommended_action'],
                        'model': f"groq:{GROQ_EXPLANATION_MODEL}",
                        'fallback': False,
                    }
    ...
    def _build_explanation_context(self, url: str, url_class: Dict, content_class: Dict, 
                                   ssl: Dict, vt: Dict) -> Dict[str, Any]:
        ...
        if url_class.get('is_malicious', False):
            threat_factors.append(f"URL pattern indicates malicious intent ({url_class.get('confidence', 0):.0%} confidence)")
        else:
        ...
        if content_class.get('is_phishing', False):
            threat_factors.append(f"Content shows phishing indicators ({content_class.get('confidence', 0):.0%} confidence)")
        else:
    ...
    def _fallback_content_analysis(self, html_text: str) -> Dict[str, Any]:
        ...
        phishing_keywords = ['verify your account', 'confirm your identity', 'urgent action required', 
                            'suspended account', 'click here immediately', 'prize winner']
    ...
    async def analyze_with_llm(self, url: str, html_content: str, ssl_analysis: Dict, 
                              vt_analysis: Dict) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            final_result = {
                ...
                'overall_assessment': {
                    'is_malicious': url_classification.get('is_malicious', False) or 
                                   content_classification.get('is_phishing', False),
                    'confidence': max(url_classification.get('confidence', 0), 
                                    content_classification.get('confidence', 0)),
