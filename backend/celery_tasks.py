...
SCAN_EXECUTOR = ThreadPoolExecutor(
    max_workers=min(_cpu_count, 4),  # ULTRA SAFE: 1x CPU cores, max 4 (was 16)
    thread_name_prefix="WebShield-UltraSafe"
)
    ...
    try:
        ...
        def process_single_scan(url_scan_pair):
            ...
            try:
                ...
                ).get(timeout=180)  # 3 minute timeout per scan
        ...
        logger.info(f"🎯 Bulk scan completed: {successful_scans} successful, {failed_count} failed out of {total_urls} total")
    ...
    try:
        with get_db_connection_with_retry() as conn:
            if conn:
                ...
                # Prepare batch data
                batch_data = [
                    (scan_id, url, 'processing', datetime.now(), user_email)
                    for url, scan_id in scan_data
                ]
...
def cleanup_old_scans():
    ...
    try:
        ...
        with get_db_connection_with_retry() as conn:
            if conn:
                ...
                if cleanup_stats['stuck_scans_recovered'] > 0:
                    logger.warning(f"⚡⚡ ULTRA-AGGRESSIVE recovery: {cleanup_stats['stuck_scans_recovered']} stuck scans recovered (20s timeout)")
                
                # 3. Clean up very old failed scans (7 days)
                failed_cutoff = datetime.now() - timedelta(days=7)
                cursor.execute(
                    "DELETE FROM scans WHERE created_at < %s AND status = 'failed'",
                    (failed_cutoff,)
                )
                cleanup_stats['old_scans_deleted'] += cursor.rowcount
        ...
        try:
            ...
            expired_keys = [k for k, v in SCAN_CACHE.timestamps.items() 
                          if current_time - v > SCAN_CACHE.ttl]
            ...
            stuck_urls = [url for url, ts in SCAN_IN_PROGRESS_TIMESTAMPS.items() 
        ...
        try:
            with get_db_connection_with_retry() as conn:
                if conn:
                    ...
                    """, (
                        datetime.now(),
                        intel_stats['feeds_updated'],
                        intel_stats['new_threats'],
                        'success'
                    ))
