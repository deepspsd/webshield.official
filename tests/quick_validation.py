...
async def test_llm_service_basic():
    ...
    try:
        async with LLMService() as llm:
            ...
            print_test("API Key Configured", has_api_key, 
                      "Set HUGGINGFACE_API_KEY in .env" if not has_api_key else "")
            return True
...
async def test_url_classification():
    ...
    try:
        async with LLMService() as llm:
            ...
            print_test("Confidence Value Valid", valid_confidence, 
                      f"Confidence: {result.get('confidence', 'N/A')}")
            print_test("Label Present", has_label, 
                      f"Label: {result.get('label', 'N/A')}")
            print_test("Model Info Present", has_model, 
...
async def test_explanation_generation():
    ...
    try:
        async with LLMService() as llm:
            ...
            print_test("Recommended Action Present", has_action,
                      f"Action: {result.get('recommended_action', 'N/A')[:50]}...")
...
async def test_chart_data_structure():
    ...
    try:
        ...
        print_test("Detection Counts Valid", valid_counts,
        ...
        print_test("ML Confidence Values Valid", valid_confidences,
                  f"URL: {url_conf:.2%}, Content: {content_conf:.2%}")
