...
class TestScanEndpoint:
    ...
    def test_scan_url_valid(self, sample_urls):
        ...
        response = client.post("/api/scan/scan", json={"url": sample_urls["safe"][0], "user_email": None})
    ...
    def test_scan_url_invalid(self):
        ...
        response = client.post("/api/scan/scan", json={"url": "not-a-valid-url", "user_email": None})
        assert response.status_code == 200
    ...
    def test_scan_url_empty(self):
        ...
        response = client.post("/api/scan/scan", json={"url": "", "user_email": None})
        assert response.status_code == 200
    ...
    def test_get_scan_result(self, sample_urls):
        ...
        # First, create a scan
        scan_response = client.post("/api/scan/scan", json={"url": sample_urls["safe"][0], "user_email": None})
...
class TestScanCache:
    ...
    def test_scan_cache(self, sample_urls):
        ...
        # First scan
        response1 = client.post("/api/scan/scan", json={"url": url, "user_email": None})
        assert response1.status_code == 200

        # Second scan (should be faster due to cache)
        response2 = client.post("/api/scan/scan", json={"url": url, "user_email": None})
